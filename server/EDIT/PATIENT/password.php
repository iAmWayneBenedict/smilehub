...
/**
 * Function to update the patient's password in the database
 *
 ...
 */
function updatePatientPassword($conn, $data) {
    // Fetch the current password hash for the patient
    $query = "SELECT PASSWORD FROM patient_table WHERE ID = ?";
    ...
    $query = "UPDATE patient_table SET PASSWORD = ? WHERE ID = ?";
    $stmt = $conn->prepare($query);
...
if (updatePatientPassword($conn, $data)) {
