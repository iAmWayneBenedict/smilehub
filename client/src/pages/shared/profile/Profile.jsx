				...
				<div>
					...
					>
						...
						<Tab key="edit" title="Edit Profile">
							<ProfileForm isView={false} selected={selected} setSelected={setSelected} />
						</Tab>
					</Tabs>
				</div>
			</div>
...
const ProfileForm = ({ isView, selected, setSelected }) => {
	...
	const {data, isSuccess, isLoading, refetch} = useQuery({
		...
		queryFn: () => EmployeesAPIManager.getEmployeeDetails({
			ID: params?.id
		})
	})
	...
	const {
			...
			defaultValues: useMemo(() => {
				...
				return  {
					...
					BIRTHDAY: parseDate(convertDateYYYYMMDD(data?.BIRTHDAY)) || today(getLocalTimeZone()), // default date (today)
					GENDER: data?.GENDER,
					ROLE: data?.ROLE,
					EMAIL: data?.EMAIL,
				}
					...
					<div style={{flex:1}} className="flex flex-col gap-5">
						<form style={{flex:1}} onSubmit={handleSubmit(onSubmitDetails)}>
							...
							>
								...
								{!isView && (
									<div>
										<Button type={"submit"} color="primary" size="lg" radius="sm" className="py-7" 								isLoading={detailsMutation.isPending}>
											{detailsMutation.isPending ? "Changing..." : "Update Information"}
										</Button>
									</div>
								)}
							</div>
						</form>
						<PasswordResetComponent isView={isView} selected={selected} setSelected={setSelected} refetch={refetch} />
					</div>
				</div>
		</div>
	);
...
const PasswordResetComponent = ({isView, selected, setSelected, refetch}) => {
	...
	return (
		<form onSubmit={handleSubmit(onSubmit)}>
			...
			{!isView && (
				<div>
					<Button type={"submit"} color="primary" size="lg" radius="sm" className="py-7">
						Change Password
					</Button>
				</div>
			)}
